...
class philosopher(threading.Thread):
  def __init__(self, fork_1, fork_2, meals_available):
    super().__init__()
    ...
    self.fork_1 = fork_1
    self.fork_2 = fork_2
    self.meals_available = meals_available

  def run(self):
    while self.meals_available.acquire():
      print(f"{self.ident}: Fork 1\n")

      self.fork_1.acquire()

      print(f"{self.ident}:Fork 2\n")

      if self.fork_2.acquire(timeout = random.randint(1,3)):

        print(f"{self.ident}:NOM\n")
        ...
        self.fork_2.release()
          ...
          print(f"{self.ident}:Thinking. Setting down left fork.\n")
          self.thought += 1

          self.fork_1.release()
          self.meals_available.release()
...
def main():
    ...
    #TODO a lock/semaphore for each fork, 5 philosopher threads
    #A philosopher can eat with two forks for rand_int(1,3)
    #A philosopher must think for at least rand_int(1,3)

    meals_eaten = threading.Semaphore(MAX_MEALS_EATEN)

    forks = [threading.Lock() for fork in range(PHILOSOPHERS)]

    philosophers = [philosopher(forks[p-1], forks[p], meals_eaten) for p in range(PHILOSOPHERS)]
    ...
    print([ph.thought for ph in philosophers])
    print([ph.ate for ph in philosophers])
